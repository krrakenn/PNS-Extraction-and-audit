...
from audit_prod_pydantic import ProductsAuditReport
...
def _thinking_json_to_products_payload(thinking_raw: str) -> dict:
    """Parse thinking JSON and return a dict containing only ``products`` for the audit prompt."""
    try:
        obj = json.loads(thinking_raw) if thinking_raw else {}
    except Exception:
        obj = {}
    ...
    for attempt in range(1, retries + 1):
        ...
        try:
            request_kwargs = dict(
                ...
                response_format={
                    "type": "json_schema",
                    "json_schema": {"name": "pns_audit_output", "schema": ProductsAuditReport.model_json_schema()},
                }
...
def run_audit(
    ...
    if not common_keys:
        raise ValueError("No matching records found across both CSVs.")
    ...
    with open(results_csv, "a", encoding="utf-8", newline="") as f_out:
        ...
        for idx, key in enumerate(common_keys, start=1):
            ...
            gt_payload = _thinking_json_to_products_payload(thinking_1)
            flash_payload = _thinking_json_to_products_payload(thinking_2)
            thinking_1_for_llm = json.dumps(gt_payload, ensure_ascii=False, indent=2)
            thinking_2_for_llm = json.dumps(flash_payload, ensure_ascii=False, indent=2)

            _audit_log(
                f"[{idx}/{total}] unique_id={unique_id} key={key} "
                f"thinking_lens=({len(thinking_1)},{len(thinking_2)}) chars "
                f"products_json_lens=({len(thinking_1_for_llm)},{len(thinking_2_for_llm)})"
            )
            _audit_log(f"  recording_url={url[:120]}{'…' if len(url) > 120 else ''}")

            checklist = _build_top_level_field_checklist(gt_payload, flash_payload)

            prompt_text = build_audit_prompt(
                url, llm_1, llm_2, thinking_1_for_llm, thinking_2_for_llm, field_checklist_grouped=checklist
            )
...
def main() -> None:
    ...
    OUTPUT_DIR = os.path.join(BASE_OUTPUT_DIR, f"products_audit_outputs_{run_ts}")
    ...
    _audit_log("main() starting (Products Audit)")
